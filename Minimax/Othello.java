...
public class OthelloSolver {
  ...
  // We want to keep these enum values so that flipping ownership is just a sign
  // change
  ...
  // Rather than having you implement a full Othello player, which is hard to fit
  // into the
  // HackerRank paradigm, we're going to just evaluate board positions according
  // to the evaluation
  // function. This has the nice property of being more specific than the choice
  // of move,
  // helping us make sure the algorithm has been implemented properly. It should
  // also be
  // robust against changes to the order that moves are explored.
  //
  // The input will be the search depth (in ply, or single moves) and a board
  // position,
  // and the output will be the value of the game in piece difference (+WIN_VAL
  // for a white win,
  // -WIN_VAL for a black win, piece difference otherwise).
  //
  // Some code has been provided for you as well if you want to plug your search
  // into an Othello
  // player; use the command-line argument "play."
  //
  // Take note of the provided functions - the gory details of capturing pieces
  // and so forth
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        minimax_value(board, WHITE_TO_PLAY, searchDepth, Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY));
  }
  ...
  static boolean capturesInDir(int[][] board, int row, int row_delta, int col, int col_delta, boolean whiteTurn) {
    ...
    // Nothing to capture if the neighbor in the right direction isn't of the
    // opposite color
    ...
    while ((scanRow >= 0) && (scanRow < NUM_COLUMNS) && (scanCol >= 0) && (scanCol < NUM_COLUMNS)
        && (board[scanRow][scanCol] != NOBODY)) {
    ...
    for (int row_delta = -1; row_delta <= 1; row_delta++) {
      for (int col_delta = -1; col_delta <= 1; col_delta++) {
        ...
        if (capturesInDir(board, row, row_delta, col, col_delta, whiteTurn)) {
          ...
          int enemyColor = (whiteTurn ? BLACK : WHITE);
          // No need to check for board bounds - capturesInDir tells us there's a friendly
          // piece
  ...
  static float minimax_value(int board[][], boolean whiteTurn, int searchDepth, float alpha, float beta) {
    ...
    if (whiteTurn) {
      return maxValue(board, searchDepth, alpha, beta);
    } else {
      return minValue(board, searchDepth, alpha, beta);
    }
  }

  static float maxValue(int board[][], int searchDepth, float alpha, float beta) {
    int winner = checkGameOver(board);
    if (winner == WHITE) {
      return 100;
    } else if (winner == BLACK) {
    ...
    // just added
    if (searchDepth == 0) {
      float result = evaluationFunction(board);
      return result;
    }

    float v = Float.NEGATIVE_INFINITY;
    ArrayList<Move> moves = generateLegalMoves(board, true);
    if (moves.size() == 0) {
      return -100;
    }
    for (Move m : moves) {
      int[][] copy = play(board, m, true);
      float temp = minValue(copy, searchDepth - 1, alpha, beta);
      v = Math.max(v, temp);
      if (v >= beta) {
        return v;
      }
  ...
  static float minValue(int board[][], int searchDepth, float alpha, float beta) {
    ...
    ArrayList<Move> moves = generateLegalMoves(board, false);
    if (moves.size() == 0) {
      return 100;
    }
    
    for (Move m : moves) {
      int[][] copy = play(board, m, false);
      float temp = maxValue(copy, searchDepth - 1, alpha, beta);
      v = Math.min(v, temp);
  ...
  // The rest of the code here is for the interactive Othello player. You don't
  // need to
  // touch it or use it.
  // ------------------------------------------------------------------------------------
  static void play() {
    ...
    while (checkGameOver(board) == NOBODY) {
      ...
      if (legalMoves.size() > 0) {
        ...
        for (Move m : legalMoves) {
          ...
          float moveVal = minimax_value(newBoard, true, DEMO_SEARCH_DEPTH, Float.NEGATIVE_INFINITY,
